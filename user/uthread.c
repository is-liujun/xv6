...
void thread_schedule(void) {
    ...
    for (int i = 0; i < MAX_THREAD; i++) {
        if (t >= all_thread + MAX_THREAD) // 如果t超除了all_thread的尾部
            t = all_thread;
        if (t->state == RUNNABLE) {
            next_thread = t;
            break;
...
void thread_create(void (*func)()) {
    ...
    t->state = RUNNABLE;
    // YOUR CODE HERE
    // 返回地址，thread_switch线程切换执行完成后返回到ra，设置成func函数，就可以在切换后执行func
    t->context.ra = (uint64)func;
    // 栈指针，将线程的栈指针指向其独立的栈，栈的生长是从高地址到低地址，所以要将sp设置为指向stack的最高地址
