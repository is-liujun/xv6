...
#define PA2PGREF_ID(p) (((p) - KERNBASE) / PGSIZE) // 由物理地址获取物理页
#define PGREF_MAX_ENTRINES PA2PGREF_ID(PHYSTOP)    // 物理页上限

int pageref[PGREF_MAX_ENTRINES]; // 每个物理页的引用数，
...
// 写时复制一个新的物理页面返回
//  如果该物理页的引用数>1, 则将引用计数-1，并分配一个新的物理页面返回
